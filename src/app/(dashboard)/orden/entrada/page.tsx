...
import { NotaEntrada, Response } from "@/interfaces/responsefinal.interface";

export default async function NotaEntradaPage() {
  const data = await ApiRequest<Response<NotaEntrada[]>>({
    metod: "get",
    ...
    <div className="w-full h-full p-8 flex flex-col bg-gray-100  dark:bg-gray-900">
      ...
      <div className="grid grid-cols-1 items-center mt-6 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card
          title="En proceso"
          icon={
            <LuClock4 size={28} className="text-white dark:text-orange-400" />
          }
          description="20"
          extra="Recibo pendiente"
          className={{
            container:
              "bg-yellow-100 shadow-lg dark:bg-gray-800 dark:border dark:border-gray-700 dark:hover:shadow-orange-400/10 dark:transition-all",
      ...
      <Tabs headers={["Catalogo de Entradas"]} className="mt-6">
        <ListarNotaEntrada data={data?.data || []}></ListarNotaEntrada>
      </Tabs>
    </div>
  );
}
