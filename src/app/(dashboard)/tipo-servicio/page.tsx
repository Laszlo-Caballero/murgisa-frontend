...
import CrearTipoServicio from "@/modules/tipo-servicio/crear/CrearTipoServicio";
import ListarTipoServicio from "@/modules/tipo-servicio/listar/ListarTipoServicio";
import { ApiRequest } from "@/libs/api";
import { Response, TipoServicio } from "@/interfaces/responsefinal.interface";

export default async function TipoServicioPage() {
  const data = await ApiRequest<Response<TipoServicio[]>>({
    metod: "get",
    ...
    <div className="w-full h-full p-8 flex flex-col bg-gray-100 dark:bg-gray-900">
      <header className="flex md:flex-row flex-col md:items-center relative gap-x-4 rounded-xl p-5 bg-gradient-to-r from-purple-600 to-purple-700 ">
        ...
        <div className="flex flex-col text-white">
          <p className="font-bold text-3xl">Tipo de Servicios</p>
          <p className="text-sm">
            Gestiona los tipos de servicios disponibles para tus clientes
          </p>
        </div>
        ...
        <ButtonModal
          ...
          modal={<CrearTipoServicio />}
        >
          <FiPlus size={15} />
          Nueva Tipo de Servicio
        </ButtonModal>
      </header>
      <div className="grid grid-cols-1 items-center mt-6 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card
          title="Total de Tipos de Servicios"
          icon={
            <LuBriefcase
              size={28}
          ...
          className={{
            container:
              "bg-purple-100 shadow-lg dark:bg-gray-800 dark:border dark:border-gray-700 dark:hover:shadow-blue-400/10 dark:transition-all",
        ...
        />
        <Card
          title="Tipos de Servicios Activos"
          icon={
            <LuCircleCheckBig
              size={28}
        ...
        <Card
          title="Tipos Asignados a Servicios"
          icon={
            <LuChartColumnIncreasing
              size={28}
      ...
      <Tabs headers={["Catálogo de Tipo de Servicios"]} className="mt-6">
        <ListarTipoServicio data={data?.data || []} />
      </Tabs>
    </div>
  );
}
