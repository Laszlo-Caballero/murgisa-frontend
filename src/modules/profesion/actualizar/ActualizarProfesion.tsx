...
interface ActualizarProfesionProps extends ModalProps {
  id: number;
}

export default function ActualizarProfesion({
  onClose,
  id,
}: ActualizarProfesionProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  >({
    mutationFn: async (data, urlApi, token) => {
      const res = await axios.patch(`${urlApi}/profesion/${id}`, data, {
        headers: {
  ...
  const { data: profesiones, isLoading: isLoadingProfesiones } = useQuery<
    Response<Profesion>
  >({
    queryFn: async (url, token) => {
      const response = await axios.get(`${url}/profesion/${id}`, {
  ...
  useEffect(() => {
    if (profesiones) {
      const data = profesiones.data;
      setValue("titulo", data.titulo);
      setValue("descripcion", data.descripcion);
    }
  }, [profesiones, setValue]);
  ...
  return (
    ...
    >
      {(isLoading || isLoadingProfesiones) && <Load />}
