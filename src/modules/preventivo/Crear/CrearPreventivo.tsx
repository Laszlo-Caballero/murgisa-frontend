  ...
  return (
    ...
    >
      <header className="flex items-center gap-x-3">
        <LuShield size={40} className="text-orange-600" />
        <div className="flex flex-col">
          <p className="text-xl font-semibold dark:text-white">
            Agregar Mantenimiento Preventivo
          </p>
          <p className="text-sm text-gray-500">
            Completa los datos para registrar un nuevo mantenimiento preventivo
            en el sistema
          </p>
        </div>
      </header>
        {(isLoading || loadingMantenimientoPreventivo) && <Load />}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4 dark:text-gray-300">
                <InputDate
                  label="Fecha de Mantenimiento"
                  icon={<CiCirclePlus />}
                  placeholder="Selecciona una fecha"
                  error={errors.fechaMantenimiento?.message}
                  onChange={(date) => setValue("fechaMantenimiento", date)}
                ...
                <Select
                  ...
                  onChange={(value) => setValue("Prioridad", value.value)}
                ...
                <Select
                  ...
                  options={recurso?.data?.map((r) => ({
                    label: r.nombre,
                    value: r.idRecurso.toString(),
                  }))}
                  onChange={(value) => setValue("recurso", {
                    value: value.value,
                ...
                <Select
                  ...
                  options={personal?.data?.map((p) => ({
                    label: p.nombre,
                    value: p.idPersonal.toString(),
                  }))}
                  onChange={(value) => setValue("personal", {
                    value: value.value,
                    label: value.label?.toString() || "",
                  })}
                ...
                <Select
                  ...
                  onChange={(value) => setValue("horario", {
                    value: value.value,
                    label: value.label?.toString() || "",
                  })}
                  value={watch("horario")}
