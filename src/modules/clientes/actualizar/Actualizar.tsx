...
export default function ActualizarCliente({ onClose, id }: CrearClienteProps) {
  ...
  const { data: clienteData, isLoading: loadingCliente } =
    useQuery<Response<Cliente> | null>({
      queryFn: async (url, token) => {
        const response = await axios.get<Response<Cliente>>(
          `${url}/cliente/${id}`,
          {
            headers: {
  ...
  }, [clienteData?.data, loadingCliente, setValue]);
  ...
  return (
    ...
    >
      {(isLoading || loadingClientes || loadingCliente) && <Load />}
